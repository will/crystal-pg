...
module PG
  module Decoders
    struct RangeDecoder(T)
      ...
      FLAG_LOWER_INFINITY   = 0b00000100
      FLAG_UPPER_INCLUSIVE  = 0b00001000
      ...
      private def decode_boundary(io, oid, header, type : {{ key.id }}.class )
        if header.empty
          \{% if T.nilable? %}
          nil
          \{% else %}
      ...
      def decode(io, bytesize, oid)
        ...
        if header.empty
          ...
          lower = decode_boundary(io, oid, header, T)
          upper = decode_boundary(io, oid, header, T)
